<form id="update_customer_details_form" action="/customer/update" method="get">
    ...
    <div class="custom_fields_half associate">
        <div class="custom_Request_fields_half">
            ...
            <select required class="AddressCountryNew country_select" name="country" @if($customer != null) data-country-select="{{ $customer->country }}"  data-province-select="{{$customer->state}}" @endif>
    ...
    <div class="full_width_iput">
        <div class="custom_Request_fields_half">
            ...
            <input type="text" required="" id="autocomplete"  class="autocomplete" value="{{ $customer->address1 }}" placeholder="Street and number, P.O.box C/O">
    ...
    <div class="Complete-address">
        ...
        <div id="" class="custom_fields_half full_width_iput">
            <div class="custom_Request_fields_half adj">
                ...
                {{--                        </select>--}}
                <input type="text" class="AddressProvinceNew2" name="province" id="administrative_area_level_1" autocomplete="address-level1">
            </div>
        </div>
            ...
            <select class="" name="billing_address" id="billing_address" >
                @foreach($addresses as $address)
                    @if($address->address_type == "Billing")
                        <option @if($address->default == 1) selected @endif value="{{$address->id}}"> {{$address->first_name.' '.$address->last_name.', '.$address->address1.' '.$address->address2.' '.$address->city.' '.$address->state.' '.$address->country.','.$address->postcode}}  @if($address->default == 1) (Default) @endif </option>
                    @endif
                @endforeach
            </select>
        </div>
